...
## Continuity conformance contract — 2026-08-13

- Added `docs/continuity-contract.md` as the normative, CodeWave-owned acceptance boundary for daemon/SQLite continuity under retries, races, external termination, restart, and reconstruction.
- Translated ten invariants into agent-workspace terms: scoped identifiers, one canonical run truth, exact authorization and target scope, one daemon-owned commit path, minimum attributable history, atomic semantic idempotency, recovery equivalence, causal traceability, explicit time/external input, and versioned authority/adapters.
- Froze six vector families before backend adaptation: authority/revocation races, durable semantic idempotency, unqueued one-active-run conflict, external-kill crash boundaries, deterministic reconstruction/version failure, and content separation with a privacy-limited audit projection.
- Kept the boundary local-first and SQLite-native. The contract does not import or require Restate, PostgreSQL, DBOS, Docker, the parked research repository, or its private implementation.
- Defined explicit non-claims for external side effects, database-file attackers, backups/WAL erasure, legal compliance, provider correctness, production SLOs, demand, and product readiness.
- Implemented the executable gate as `npm run check:continuity` → `scripts/validate-continuity.mjs`, with generated machine-readable run evidence at `.codewave/qa/continuity-dogfood-2026-08-13/backend/validated-post-fix.json`. The `.codewave/` evidence is intentionally ignored and non-authoritative unless the validator produced it for the identified tree and passed CW-CV1–CW-CV6 plus CW-I1–CW-I10.
