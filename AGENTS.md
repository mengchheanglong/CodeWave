...
- keep built-in provider IDs explicit and validate custom profiles as lowercase `acp.*`; render provider choices from the daemon registry, create local executable profiles disabled, and never probe or spawn them before explicit enablement
...
- keep project tasks on readable `codewave/task-*` branches inside daemon-managed worktrees; require an exact clean Git root, revalidate worktree/branch/common-Git identity, suppress repository hooks and signing, hide Git/daemon control paths, bound review output, fence stale/incomplete reviews and provider-run preparation in both directions, close accepted/reverted tasks to later provider runs, and never merge the project branch implicitly
