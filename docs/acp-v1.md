...
The built-in OpenCode ACP path and dynamic `acp.*` profiles use `@codewave/provider-acp`, a profile-driven adapter that owns compatibility probing, launch, stderr bounds, and process cleanup while delegating wire semantics to this runtime. Gemini retains its thin built-in wrapper for parity. Provider policy v2 persists validated custom launch descriptors, while the UI requires explicit local-executable trust and creates every new profile disabled.

## Executable evidence

`npm run check:acp` exercises the stable runtime with a deterministic app-style ACP fixture and a real daemon cancellation path. `npm run check:acp-provider` uses a second, hand-written agent to prove profile-only launch, probe coalescing, capability mapping, run/cancel behavior, stderr ceilings, and child cleanup. `npm run check:acp-custom` proves custom-profile validation, disabled fail-closed behavior, enablement, real runs, restart persistence/resume, and updates through the daemon contract. `npm run check:transport` retains Gemini/OpenCode normalization parity. These gates must pass before changing ACP lifecycle behavior.
